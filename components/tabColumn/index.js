...
function grabData(dataSource) {
    ...
    dataSource.data.forEach((item, ii) => {
      if (item.title && item.content) {
        ...
        _contents.push(
          ...
          : {
            title: item.content,
            itemClass: !ds.multipy ? select != ii ? 'none' : '' : ''
          }
        )
      }
    })
  ...
  const contents = ds.multipy ? {
    $$id: '__mytabcontent',
    data: _contents,
    type: ds.multipy == 'swiper' ? {
      is: ds.multipy,
      current: 0,
      'easing-function': 'linear'
    } : {
      is: ds.multipy || 'scroll',
      'scroll-y': true,
      'scroll-into-view': '',
...
Component({
  ...
  methods: {
    ...
    menusFunc: function(data, idx, inst) {
      console.log(data, idx, inst)
    ...
    tabIndex: function(e, param, inst) {
      if (param) {
        ...
        $menus.data = this.menusFunc($menus.data, idx, inst)

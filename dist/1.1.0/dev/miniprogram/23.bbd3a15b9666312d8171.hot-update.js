...
var savePrevSelect = []; //用来存储点击过的key值，menu。主要想实现，上一个值与当前值相比是大是小，来区别动画是哪个方向
...
function grabData(dataSource) {
  ...
  var type = ds.type;
  return { menus: menus, contents: contents, type: type };
}
  ...
  methods: {
    ...
    tabIndex: function tabIndex(e, param) {
      ...
      if (param) {
        ...
        savePrevSelect.length > 1 ? savePrevSelect = savePrevSelect.slice(1) : savePrevSelect;

        $menus.data = $menus.data.map(function (item, ii) {
          if (item.itemClass) item.itemClass = '';
          if (ii == idx) {
            savePrevSelect.push(idx);
            item.itemClass = savePrevSelect.length > 1 && _this.data.$type == 'default' ? savePrevSelect[0] > savePrevSelect[1] ? 'xx active' : 'active yy' : 'active';
          }
          return item;
        });

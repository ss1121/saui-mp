...
Pager({
  ...
  onSub: function(e, parma, inst) {
    ...
    if (this.videoAdShow) {
      ...
      console.log(e.detail.value, this)
      this.uploadingData(value)
    }
  },
  ...
  uploadingData: function(value) {
    ...
    if (value) {
      db.collection('messages').add({
        data: {
          from: this.nimingStatus ? '匿名' : app.globalData.userInfo.nickName,
          logo: this.nimingStatus ? '/images/logo-2.png' : app.globalData.userInfo.avatarUrl,
  ...
  getAllListDate: function (data) {
    ...
    data.map( item => {
      output.push({
        img: {
          src:  this.nimingStatus ? '/images/logo-2.png' : item.logo,
          itemClass: 'item-logo'
        },
        body: [
          {
            title: [
              {
                title: this.nimingStatus ? '匿名' : item.from
              },
              {
                title: item.date,

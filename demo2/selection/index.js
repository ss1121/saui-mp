...
Pager({
  data: {
    ...
    selectionBody: Pager.list({
      ...
      listClass: 'selection-body bg-fff disN-list none-0',
      itemClass: 'hei-p100'
    }),
    tipx: {
      $$id: 'tipx'
  ...
  onTap:  function(e, param, inst) {
    ...
    // inst.siblings().addClass('none')
    // inst.removeClass('none')
    ...
    setTimeout(() => {
      ...
      if (this.savePrevSelect.length > 1){
        if (this.savePrevSelect[1] != idx){
          this.updateDataFunc(inst, $selectionBody, idx)
        }
      }
      else {
        this.updateDataFunc(inst, $selectionBody, idx)
  ...
  updateDataFunc: function(inst, $selectionBody, idx) {
    //更新头部列表，及更新对应body内容
    ...
    util.onMenus(inst, idx, this.savePrevSelect, ' ')
    console.log(idx, this.savePrevSelect, $selectionBody)
    $selectionBody.addClass('none-'+idx)
    $selectionBody.removeClass('none-'+this.savePrevSelect[0])
    // $selectionBody.findAndUpdate({id: idx}, function(res){
    //   console.log(res)
    //   res.itemClass = ''
    //   return res
    // })
  ...
  forCloudData: function(name, getElementsById, param = {} ) {
    ...
    if (type == 'single') {
      ...
      if (this.xxoo[name].length > 0 && this.xxoo[name].indexOf(id) < 0) {
        ...
        .then(res => {
          return inst.updateItems({data:res, id:data.indexOf(id)})
        })
      }
    }
    else {
      ...
      data.map(async(item, ii) => {
        ...
        .then(res => {
          ...
          return inst.updateItems({data:res, id:ii})
        })
      })
  ...
  onLoad: function() {
    ...
    that.forCloudData('inland', 'inlandx')
    // setTimeout(() => {
    //   that.forCloudData('overseas', 'overseasx')
    // }, 1000);
  },
...
async function getCloudData(param){
  try {
    if (param) {
      ...
      console.log(cloudData, cfrl)
      return selectionAllAdapter(cloudData, cfrl)
    }
  } catch (error) {
    console.error(error);
  }
...
//       //     }
//       //   }
//       //   tabChoose.updateItems([selectionAllAdapter(cloudData, name)], letter.indexOf(name))
//       //   storeData.push(selectionAllAdapter(cloudData, name))
//       // }
//       // else {
...
//             }
//             return i
//             // tabChoose.updateItems([selectionAllAdapter(cloudData, letter[i])], i)
//             // storeData.push(selectionAllAdapter(cloudData, letter[i]))
//           }
//         }
...
//       }
//     })
//     this.inlandData.push(selectionAllAdapter(inlandData, letter[i]))
//   }
...
//       }
//     })
//     this.overseasData.push(selectionAllAdapter(overseasData, letter[i]))
//   }
// },
...
//     }
//   })
//   tabChoose.updateItems([selectionAllAdapter(inlandData, name)], letter.indexOf(name))
// },